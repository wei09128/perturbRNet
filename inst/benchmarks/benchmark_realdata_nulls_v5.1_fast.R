...
summarize_fit <- function(fit, null_type, draw_id) {
  ...
  gross <- counter$gross_input[is.finite(counter$gross_input)]
  ...
  data.frame(
    ...
    strong_counterdirection_fraction = if (length(cvalue)) mean(cvalue >= 0.1) else NA_real_,
    active_balanced_fraction = if (nrow(counter)) mean(counter$balance_fraction >= 0.8 & counter$gross_input >= 0.1, na.rm = TRUE) else NA_real_,
    variance_explained = if (length(variance)) mean(variance) else NA_real_,
    rank_fraction = NA_real_,
    effective_rank_fraction = NA_real_,
...
metrics <- c(
  ...
  "strong_counterdirection_fraction", "active_balanced_fraction", "variance_explained"
)
