...
# ============================================================
# perturbRNet Benchmark 3.1: stressed cell counts to counterdirection
# ============================================================
# Usage:
#   Rscript benchmark_end_to_end_counts_v3.1.R quick
#   Rscript benchmark_end_to_end_counts_v3.1.R full
#
...
output_dir <- paste0("benchmark_end_to_end_v3.1_", mode)
...
make_network <- function(cancellation_strength, footprint_size, seed) {
  set.seed(seed)
  ...
  r1_unique <- sample(available, footprint_size)
  r2_unique <- sample(setdiff(available, r1_unique), footprint_size)
  prior <- rbind(
  ...
  for (r in regulators[-c(1, 2)]) {
    targets <- sample(available, max(20L, footprint_size))
...
simulate_cells <- function(
  ...
  baseline <- count_scale * exp(stats::rnorm(n_genes, log(1.5), 1.0))
  baseline <- pmax(baseline, 0.03)

  # Unmodeled treatment-responsive genes are not generated by W. They test
  # whether PRN can recover its regulatory signal amid unrelated biology.
  nuisance_effect <- rep(0, n_genes)
  nuisance_n <- round(n_genes * nuisance_fraction)
  if (nuisance_n > 0) {
    nuisance_index <- sample(seq_len(n_genes), nuisance_n)
    nuisance_effect[nuisance_index] <- stats::rnorm(nuisance_n, 0, 0.35)
  }
  ...
  for (donor in seq_len(donor_n)) {
    for (condition in c("control", "treated")) {
      part <- part + 1L
      treatment_effect <- if (condition == "treated") {
        truth$true_effect + nuisance_effect +
          stats::rnorm(n_genes, 0, response_heterogeneity)
      } else {
        0
      }
      gene_mean <- baseline *
...
run_one <- function(
  ...
  scenario_text <- paste(
    ...
    response_heterogeneity, footprint_size, count_scale, nuisance_fraction,
    cancellation_strength, replicate_id,
    sep = "_"
  )
  seed <- seed_base + sum(utf8ToInt(scenario_text)) + replicate_id * 1009L
  truth <- make_network(cancellation_strength, footprint_size, seed)
  sim <- simulate_cells(
    truth, donor_n, cells_per_group, biological_dispersion,
    response_heterogeneity, count_scale, nuisance_fraction, seed + 1L
  )
  ...
  scenario <- list(
    ...
    response_heterogeneity = response_heterogeneity,
...
if (mode == "quick") {
  grid <- expand.grid(
    donor_n = c(3, 8),
    cells_per_group = c(30, 100),
    biological_dispersion = 0.35,
    response_heterogeneity = c(0.10, 0.50),
    footprint_size = c(20, 60),
    count_scale = c(0.35, 1.00),
    nuisance_fraction = 0.20,
    cancellation_strength = c(0.08, 0.30),
    replicate_id = seq_len(2),
    KEEP.OUT.ATTRS = FALSE
  )
} else {
  grid <- expand.grid(
    donor_n = c(3, 6, 12),
    cells_per_group = c(20, 60, 200),
    biological_dispersion = c(0.15, 0.40, 0.70),
    response_heterogeneity = c(0.05, 0.25, 0.60),
    footprint_size = c(15, 40, 100),
    count_scale = c(0.25, 0.60, 1.50),
    nuisance_fraction = c(0.10, 0.30),
    cancellation_strength = c(0.05, 0.15, 0.40),
...
for (i in seq_len(nrow(grid))) {
  ...
  pieces[[i]] <- tryCatch(
    ...
    error = function(e) data.frame(
      ...
      response_heterogeneity = grid$response_heterogeneity[i],
      footprint_size = grid$footprint_size[i],
      count_scale = grid$count_scale[i],
      nuisance_fraction = grid$nuisance_fraction[i],
...
factor_names <- c(
  "donor_n", "cells_per_group", "biological_dispersion",
  "response_heterogeneity", "footprint_size", "count_scale",
  "cancellation_strength"
)
