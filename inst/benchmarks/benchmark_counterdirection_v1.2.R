...
# ============================================================
# perturbRNet Benchmark 1.2: counterdirection recovery
# ============================================================
# Usage:
...
make_truth <- function(overlap_fraction, cancellation_strength, seed) {
  ...
  # Give background regulators distinct positive activities so activity-rank
  # recovery is identifiable. Positive signs ensure they do not introduce
  # additional true counterdirection motifs.
  activity <- c(
...
run_one <- function(
  ...
  true_target_count <- table(truth$prior$source)
  true_column_norm <- sqrt(as.numeric(true_target_count))
  names(true_column_norm) <- dimnames(true_target_count)[[1]]
...
if (mode == "quick") {
  grid <- expand.grid(
    ...
    flip_fraction = c(0, 0.01, 0.05, 0.15),
  ...
  grid <- expand.grid(
    ...
    flip_fraction = c(0, 0.01, 0.05, 0.10, 0.20),
...
ideal <- ok_results[
  ok_results$donor_n == max(ok_results$donor_n) &
    ok_results$noise_sd == min(ok_results$noise_sd) &
    ok_results$drop_fraction == 0 &
    ok_results$flip_fraction == 0 &
    ok_results$overlap_fraction == min(ok_results$overlap_fraction) &
    ok_results$cancellation_strength == max(ok_results$cancellation_strength),
  , drop = FALSE
]

hard <- ok_results[
  ok_results$donor_n == min(ok_results$donor_n) &
    ok_results$noise_sd == max(ok_results$noise_sd) &
    ok_results$drop_fraction == max(ok_results$drop_fraction) &
    ok_results$flip_fraction == max(ok_results$flip_fraction) &
    ok_results$overlap_fraction == max(ok_results$overlap_fraction) &
    ok_results$cancellation_strength == min(ok_results$cancellation_strength),
  , drop = FALSE
]
...
summary_lines <- c(
  ...
  paste0("Ideal-condition mean AUPRC: ", round(mean(ideal$auprc), 4)),
  paste0("Hard-condition mean AUPRC: ", round(mean(hard$auprc), 4)),
  paste0("Mean target coverage: ",
         round(mean(ok_results$target_coverage, na.rm = TRUE), 4)),
  paste0("Mean counter-target coverage: ",
         round(mean(ok_results$counter_target_coverage, na.rm = TRUE), 4))
)
